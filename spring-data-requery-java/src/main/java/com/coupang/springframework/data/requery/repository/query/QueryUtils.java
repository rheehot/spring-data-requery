...
import com.coupang.springframework.data.requery.domain.RequerySort;
...
/**
 * com.coupang.springframework.data.requery.repository.query.QueryUtils
 *
 * @author debop
 * @since 18. 6. 14
 */
public abstract class QueryUtils {
    ...
    /**
     * Returns the order clause for the given {@link Sort.Order}. Will prefix the clause with the given alias if the referenced
    ...
    /**
     * Creates a criteria API {@link javax.persistence.criteria.Order} from the given {@link Sort.Order}.
     *
    ...
    /**
     * Check any given {@link RequerySort.RequeryOrder#isUnsafe()} order for presence of at least one property offending the
    ...
    private static void checkSortExpression(Sort.Order order) {
        ...
        if (order instanceof RequerySort.RequeryOrder && ((RequerySort.RequeryOrder) order).isUnsafe()) {
            return;
        }
